...
class Calendar extends React.Component {
  ...
  onPick = (date, whichMonth) => {
    ...
    if (whichMonth !== 0)
      whichMonth === 1 ? this.nextMonth(pick(date)) : this.preMonth(pick(date));
    else pick(date)();
  };
  ...
  render() {
    const { value, year, month, dirty, inputValue } = this.state;
    ...
    const calendarClass = classnames({ calendar: true });
    const {
      onInputChange,
      onPick,
      preYear,
            ...
            >
              ...
              >
                <div style={{ position: 'absolute', left: 4, top: 6 }}>
                  <span onClick={preYear}>&lt;</span>
                </div>
                <div style={{ position: 'absolute', left: 26, top: 6 }}>
                  <span onClick={preMonth}>&lt;&lt;</span>
                </div>
                <div
                  style={{ textAlign: 'center', paddingTop: 4, fontSize: 12 }}
                >
                  {year} - {month + 1}
                </div>
